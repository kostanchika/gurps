...
{
    public class GetRecievedFriendRequestsUseCase : IGetRecievedFriendRequestsUseCase
    {
        private readonly IRepository<FriendshipEntity> _friendshipRepository;
        private readonly IRepository<UserEntity> _userRepository;
        private readonly IMapper _mapper;
        private readonly ILogger<IGetRecievedFriendRequestsUseCase> _logger;

        public GetRecievedFriendRequestsUseCase(
            IRepository<FriendshipEntity> friendshipRepository,
            IRepository<UserEntity> userRepository,
            IMapper mapper,
            ILogger<IGetRecievedFriendRequestsUseCase> logger
        )
        {
        ...
        {
            _logger.LogInformation("Start getting recieved friend requests for user with Login = {Login}", login);
            ...
            var recievedFriendshipRequests = await _friendshipRepository.GetBySpecificationAsync(
                new UserRecievedFriendRequestsSpecification(
                    user.Id,
            ...
            _logger.LogInformation("Successfully got recieved friend requests for user with Login = {Login}", login);

            return _mapper.Map<IEnumerable<UserDto>>(recievedFriendshipRequests.Select(r => r.Initiator));
        }
    }
}
